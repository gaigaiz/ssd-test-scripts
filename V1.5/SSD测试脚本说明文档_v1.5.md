...
│       ├── _build_test_items_panel()  # 测试项选择面板（7项复选框+全选/清空）
│       ├── _build_params_panel()      # 参数配置面板（Notebook 6个标签页）
...
**数据完整性验证原理**：
- 休眠前写入随机数据文件并记录 SHA-256 校验和
- 休眠/唤醒过程中，如果 SSD 固件在待机/恢复时出现异常，可能导致：
  - 磁盘标识变化（/dev/disk/by-id/ 链接丢失）
  - 分区表损坏（GPT 头/备份头不一致）
  - 文件系统元数据损坏（fsck 报错）
  - 用户数据损坏（SHA-256 校验和不匹配）
  - SMART 介质错误增加
- 通过以上四项检查全面验证 SSD 在操作系统中断场景下的稳定性

**关键设计决策**：
1. **次级驱动器假设**：OSINT 测试假设待测 SSD 是次级驱动器（非系统盘），系统盘需要支持 S3/S4 休眠唤醒。待测盘上创建独立分区和文件系统用于测试
2. **活跃 IO 休眠**：默认在 fio 活跃读写时触发休眠，模拟真实业务场景下系统休眠的情况；支持 `--osint-io-idle` 切换为空闲状态休眠
3. **rtcwake 全自动唤醒**：使用 rtcwake 设置 RTC 闹钟定时唤醒，无需人工按电源键，适合长期循环自动化测试
4. **四项唤醒后检查**：磁盘标识 + 分区/文件系统 + 数据完整性 + SMART，全面覆盖操作系统中断可能导致的各类故障
5. **最终功能测试**：全部循环后执行容量 + SMART + 顺序读写性能，确认长期休眠唤醒循环后 SSD 功能正常、性能无衰减
...
├─────────────────────────────────────────────────────────┤
│ 状态栏：就绪 | 版本 v1.4.0 | 仅标准库                    │
└─────────────────────────────────────────────────────────┘
```

**核心功能**：
1. **设备自动扫描**：`lsblk -J` + `nvme list -o json` 双源扫描，自动过滤 loop/ram/zram/sr 设备，显示型号/容量/接口/SN/FW
2. **测试项勾选**：7 个测试项独立复选框，支持全选/清空，默认勾选 SMART+容量（非破坏性）
3. **参数可视化配置**：6 个 Notebook 标签页（通用/固件/性能/电源循环/SPOR/OSINT），每个参数对应输入框/下拉框/复选框，实时命令预览
4. **实时日志**：子进程 stdout 逐行读取，按关键字自动着色（error=红/warning=橙/success=绿/header=蓝），自动滚动到底部
5. **进度追踪**：从日志解析 `[x/y] 开始测试` 模式，计算整体进度百分比，显示当前测试项名称
6. **一键停止**：`terminate()` → 等待 5s → `kill()` 两级停止，确保子进程被正确终止
7. **配置持久化**：保存/加载 JSON 配置文件，包含设备、测试项、所有参数，方便重复测试
8. **日志导出**：将实时日志导出为 txt 文件，便于离线分析
9. **权限自动处理**：非 root 启动时自动检测，提示使用 pkexec 提权执行测试
...
**关键设计决策**：
1. **subprocess 调用而非直接调用**：GUI 通过命令行调用核心逻辑，而非直接 import 调用 main()。好处：①两种模式行为完全一致，避免 GUI 特有 bug；②子进程崩溃不影响 GUI 主进程；③停止测试时可直接 terminate 整个进程树；④命令预览直接显示实际执行命令，便于调试
2. **tkinter 而非 PyQt**：tkinter 是 Python 标准库自带，Ubuntu 默认已安装（python3-tk），无需额外依赖，符合脚本"仅标准库"的设计原则
3. **参数与命令行一一对应**：GUI 中每个参数控件直接映射到命令行参数，`build_command()` 方法构建完整命令列表，命令预览框实时显示，用户可清楚知道实际执行的命令
4. **默认勾选非破坏性测试**：启动时默认勾选 SMART 和容量（只读非破坏性），性能/固件/电源循环/SPOR/OSINT 默认不勾选，避免误操作导致数据丢失
...
**设备脱机检测**：
- 每轮测试前后检查 `/dev/nvmeXn1` 设备节点是否存在
- 使用 `blockdev --getsize64` 确认设备可正常读取容量
- 设备脱机（掉盘）时立即终止测试并报告失败

**关键设计决策**：
1. **fio verify 而非手动校验**：使用 fio 内置的 verify 机制（md5/sha256/crc32），而非自己写数据再读出来算 SHA-256。fio verify 在 block 级别校验，能精确定位错误 LBA，且性能更高（校验和计算在 fio 内部完成）
2. **写入和验证分阶段**：全磁盘模式分写入阶段（`--do_verify=0`）和读取验证阶段（`--do_verify=1`），而非写入时立即验证。这样更接近"先写满盘，再全盘读校验"的标准测试流程
3. **文件大小可自由选择**：GUI 中 256MB/1GB/4GB/16GB/32GB 五个复选框可自由勾选，命令行用 `--rw-file-sizes` 多值参数，满足不同测试场景需求
4. **长期运行先写基准数据**：24小时模式先写入 32GB 基准数据（带校验和），然后长期混合读写，最后重新验证基准数据。这样可以检测长期高负载运行后，已写入的数据是否被意外损坏（如 FTL 映射错误、写入干扰等）
5. **自动跳过超容量文件大小**：文件周期模式中，如果选定的文件大小超过设备容量，自动跳过并记录警告，避免 fio 因空间不足而失败
...
| `--osint-mount-point` | | OSINT 测试挂载点 | /mnt/ssd_osint |
| `--osint-state-file` | | OSINT 状态文件路径 | /var/lib/ssd_osint_state.json |
...
| `--rw-pattern` | | 写入数据 pattern: random/0xAA/0x55/0x00/0xFF | random |
| `--rw-verify` | | 数据校验方式: md5/sha256/crc32 | md5 |
| `--rw-long-hours` | | 长期运行测试小时数 | 24 |
| `--rw-block-size` | | 读写块大小 | 4k |
| `--rw-iodepth` | | 队列深度 | 32 |
| `--rw-numjobs` | | 并发 job 数 | 4 |
| `--rw-mixed-read-ratio` | | 长期运行混合读写读比例(%) | 70 |
...
#### 示例 12：OSINT 操作系统中断测试（S3 休眠，活跃 IO 模式）
...
#### 示例 17：读/写测试（全部模式，GUI 中也可配置）

```bash
sudo python3 ssd_test.py -d /dev/nvme0n1 -t rw --rw-mode all -y
# 依次执行：全磁盘验证 → 多文件大小周期 → 24小时长期运行
# 完整覆盖读/写测试的所有场景
```

#### 示例 18：试运行模式（不实际执行，仅验证命令和参数）

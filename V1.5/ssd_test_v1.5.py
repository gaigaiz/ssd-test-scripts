...
class SSDTestGUI:
    ...
    def _build_main_layout(self):
        ...
        # 底部：日志输出
        log_frame = ttk.LabelFrame(self.root, text="测试日志", padding=4)
    ...
    def _build_performance_tab(self):
        ...
        ttk.Label(frame, text="单项运行时间(秒)：").grid(row=row, column=0, sticky=tk.W, pady=4)
        ...
        ttk.Label(frame, text="队列深度(QD)：").grid(row=row, column=0, sticky=tk.W, pady=4)
        ...
        ttk.Checkbutton(frame, text="执行稳态预处理（SNIA 规范，耗时较长）",
                        variable=self.config_vars["precondition"]).grid(row=row, column=0, columnspan=3, sticky=tk.W, pady=4)
    ...
    def start_test(self):
        ...
        self._append_log(f"SSD 自动化测试开始 - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", "header")
        self._append_log(f"执行命令: {' '.join(cmd)}", "info")
        self._append_log("=" * 60, "header")
    ...
    def _read_output(self):
        ...
        try:
            for line in self.test_process.stdout:
                ...
                tag = "info"
                lower = line.lower()
                if any(kw in lower for kw in ["error", "fail", "失败", "错误"]):
                    tag = "error"
                elif any(kw in lower for kw in ["warning", "warn", "注意", "警告"]):
                    tag = "warning"
                elif any(kw in lower for kw in ["pass", "成功", "通过", "完成"]):
                    tag = "success"
    ...
    def _on_test_finished(self, returncode: int):
        ...
        self._append_log("=" * 60, "header")
        if returncode == 0:
            self._append_log("全部测试完成 - 结果: PASS", "success")
            self.progress["value"] = 100
            self.progress_label.config(text="完成 (PASS)")
        else:
            self._append_log(f"测试完成 - 存在失败项 (退出码: {returncode})", "error")
            self.progress_label.config(text=f"完成 (退出码 {returncode})")
        self._append_log(f"结束时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", "info")
        self._append_log("=" * 60, "header")

        self.status_var.set(f"测试完成 | 退出码: {returncode}")
        self._reset_ui_state()

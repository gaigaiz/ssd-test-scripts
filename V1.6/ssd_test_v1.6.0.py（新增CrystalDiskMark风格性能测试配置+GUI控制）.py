...
# CrystalDiskMark 风格测试预设
PERF_PROFILE_DEFAULT = "default"
PERF_PROFILE_NVME_SSD = "nvme_ssd"
PERF_PROFILE_FLASH_MEMORY = "flash_memory"
PERF_PROFILE_CUSTOM = "custom"

# CrystalDiskMark 默认测试项配置（参考 CrystalDiskMark 8.x Default profile）
# 每项: (类型 SEQ/RND, 块大小, 队列深度, 线程数)
CDM_DEFAULT_ITEMS = [
    ("SEQ", "1MiB", 8, 1),
    ("SEQ", "128KiB", 32, 1),
    ("RND", "4KiB", 32, 16),
...
# Flash Memory 预设
CDM_FLASH_MEMORY_ITEMS = [
    ("SEQ", "1MiB", 8, 1),
    ("RND", "4KiB", 32, 16),
]

# CrystalDiskMark 默认测量/间隔时间（秒）
DEFAULT_CDM_MEASURE_TIME = 5
DEFAULT_CDM_INTERVAL_TIME = 5
DEFAULT_CDM_TEST_SIZE_GIB = 64
DEFAULT_CDM_TEST_COUNT = 5
...
class TestConfig:
    ...
    # CrystalDiskMark 风格性能测试配置
    perf_profile: str = PERF_PROFILE_DEFAULT  # default / nvme_ssd / flash_memory / custom
    perf_cdm_items: List[Tuple[str, str, int, int]] = field(default_factory=lambda: list(CDM_DEFAULT_ITEMS))
    perf_measure_time: int = DEFAULT_CDM_MEASURE_TIME  # 测量时间（秒）
    perf_interval_time: int = DEFAULT_CDM_INTERVAL_TIME  # 间隔时间（秒）
    perf_test_size_gib: int = DEFAULT_CDM_TEST_SIZE_GIB  # 测试数据量（GiB）
    perf_test_count: int = DEFAULT_CDM_TEST_COUNT  # 测试次数
    perf_use_cdm_mode: bool = True  # True=使用CDM风格自定义测试项, False=使用传统三项测试
    # 电源循环测试配置
...
class PerformanceTester:
    ...
    def run_fio(self, name: str, rw: str, bs: str, qd: int,
                runtime: int, size: Optional[str] = None,
                numjobs: int = 1) -> Dict[str, Any]:
        """执行单次 fio 测试并返回解析结果。"""
        cmd = self._build_fio_cmd(name, rw, bs, qd, runtime, size, numjobs)
        self.log.info(f"  fio: {name} ({rw}, bs={bs}, qd={qd}, numjobs={numjobs}, {runtime}s)")
    ...
    # ----------------------------------------------------------
    # CrystalDiskMark 风格测试
    # ----------------------------------------------------------

    @staticmethod
    def _bs_cdm_to_fio(bs_cdm: str) -> str:
        """将 CrystalDiskMark 风格块大小转换为 fio 格式。
        1MiB -> 1m, 128KiB -> 128k, 4KiB -> 4k
        """
        bs = bs_cdm.strip().upper()
        if bs.endswith("MIB"):
            return bs[:-3] + "m"
        elif bs.endswith("KIB"):
            return bs[:-3] + "k"
        elif bs.endswith("GIB"):
            return bs[:-3] + "g"
        # 已经是 fio 格式（如 1m, 128k, 4k）
        return bs_cdm.lower()

    def _extract_cdm_metrics(self, parsed: Dict[str, Any]) -> Dict[str, Any]:
        """从 fio 解析结果中提取 CDM 风格指标（带宽/IOPS/平均延迟）。"""
        metrics = {"bw_mbps": 0.0, "iops": 0.0, "lat_us": 0.0}
        ...
        # 取有数据的一方（fio 单次测试只有一个方向有数据）
        if read_data.get("bw", 0) > 0:
            data = read_data
        elif write_data.get("bw", 0) > 0:
            data = write_data
        else:
            return metrics
        bw_kbps = data.get("bw", 0)
        metrics["bw_mbps"] = round(bw_kbps / 1000, 3)  # CDM 使用 1,000,000 bytes/s
        metrics["iops"] = round(data.get("iops", 0), 1)
        # 平均延迟（clat mean，ns -> us）
        clat = data.get("clat_ns", {})
        lat_ns = clat.get("mean", 0)
        metrics["lat_us"] = round(lat_ns / 1000, 2)
        return metrics

    def run_cdm_test_item(self, item_type: str, bs_cdm: str, qd: int,
                           threads: int, label: str) -> Dict[str, Any]:
        """执行单个 CrystalDiskMark 风格测试项（读+写）。

        Args:
            item_type: "SEQ" 或 "RND"
            bs_cdm: CrystalDiskMark 风格块大小（如 1MiB, 128KiB, 4KiB）
            qd: 队列深度
            threads: 线程数（fio numjobs）
            label: 状态标签（FOB / STEADY）

        Returns:
            包含 read 和 write 指标的字典
        """
        bs_fio = self._bs_cdm_to_fio(bs_cdm)
        rw_read = "read" if item_type == "SEQ" else "randread"
        rw_write = "write" if item_type == "SEQ" else "randwrite"
        measure_time = self.cfg.perf_measure_time
        interval = self.cfg.perf_interval_time

        self.log.info(f"  [{label}] {item_type} {bs_cdm} (Q={qd}, T={threads}) "
                      f"测量={measure_time}s, 间隔={interval}s")
        ...
        # 读测试
        read_parsed = self.run_fio(
            f"{label}_CDM_{item_type}_{bs_cdm}_Q{qd}_T{threads}_read",
            rw_read, bs_fio, qd, measure_time, numjobs=threads
        )
        result["read"] = self._extract_cdm_metrics(read_parsed)
        result["read"]["raw"] = read_parsed

        # 间隔等待
        if interval > 0 and not self.cfg.dry_run:
            time.sleep(interval)

        # 写测试
        write_parsed = self.run_fio(
            f"{label}_CDM_{item_type}_{bs_cdm}_Q{qd}_T{threads}_write",
            rw_write, bs_fio, qd, measure_time, numjobs=threads
        )
        result["write"] = self._extract_cdm_metrics(write_parsed)
        result["write"]["raw"] = write_parsed

        return result

    def run_cdm_profile(self, label: str) -> Dict[str, Any]:
        """执行完整 CrystalDiskMark 风格测试配置。
        ...
        Returns:
            包含所有测试项结果的字典
        """
        items = self.cfg.perf_cdm_items
        profile_name = self.cfg.perf_profile
        self.log.info("-" * 50)
        self.log.info(f"[{label}] CrystalDiskMark 风格测试 (Profile: {profile_name})")
        self.log.info(f"  测试项数: {len(items)}, 测量时间: {self.cfg.perf_measure_time}s, "
                      f"间隔: {self.cfg.perf_interval_time}s, 测试次数: {self.cfg.perf_test_count}")
        self.log.info("-" * 50)

        result = {
            "profile": profile_name,
            "measure_time": self.cfg.perf_measure_time,
            "interval_time": self.cfg.perf_interval_time,
            "test_count": self.cfg.perf_test_count,
            "test_size_gib": self.cfg.perf_test_size_gib,
            "items": [],
        }

        for idx, (item_type, bs, qd, threads) in enumerate(items):
            self.log.info(f"  --- 测试项 {idx + 1}/{len(items)}: {item_type} {bs} (Q={qd}, T={threads}) ---")
            item_result = self.run_cdm_test_item(item_type, bs, qd, threads, label)
            result["items"].append(item_result)

        # 输出 CrystalDiskMark 风格汇总
        self._print_cdm_summary(result, label)
        return result

    def _print_cdm_summary(self, cdm_result: Dict[str, Any], label: str):
        """以 CrystalDiskMark 日志风格输出测试结果汇总。"""
        self.log.info("")
        self.log.info("=" * 60)
        self.log.info(f"  CrystalDiskMark 风格测试结果 [{label}]")
        self.log.info("=" * 60)
        self.log.info("  * MB/s = 1,000,000 bytes/s")
        self.log.info("  * KB = 1000 bytes, KiB = 1024 bytes")
        self.log.info("")

        # [Read]
        self.log.info("  [Read]")
        for item in cdm_result["items"]:
            r = item["read"]
            line = (f"  {item['type']:3s} {item['block_size']:6s} "
                    f"(Q={item['qdepth']:2d}, T={item['threads']:2d}): "
                    f"{r.get('bw_mbps', 0):>10.3f} MB/s "
                    f"[{r.get('iops', 0):>10.1f} IOPS] "
                    f"<{r.get('lat_us', 0):>8.2f} us>")
            self.log.info(line)
        ...
        for item in cdm_result["items"]:
            ...
            line = (f"  {item['type']:3s} {item['block_size']:6s} "
                    f"(Q={item['qdepth']:2d}, T={item['threads']:2d}): "
                    f"{w.get('bw_mbps', 0):>10.3f} MB/s "
                    f"[{w.get('iops', 0):>10.1f} IOPS] "
                    f"<{w.get('lat_us', 0):>8.2f} us>")
            self.log.info(line)

        self.log.info("")
        self.log.info(f"  Profile: {cdm_result['profile']}")
        self.log.info(f"  Test: {cdm_result['test_size_gib']} GiB (x{cdm_result['test_count']})")
        self.log.info(f"  Time: Measure {cdm_result['measure_time']} sec / Interval {cdm_result['interval_time']} sec")
        self.log.info(f"  Date: {datetime.now().strftime('%Y/%m/%d %H:%M:%S')}")
        self.log.info("=" * 60)
        self.log.info("")
    ...
    def run_fob_test(self) -> Dict[str, Any]:
        ...
        # 性能测试
        if self.cfg.perf_use_cdm_mode:
            result["cdm"] = self.run_cdm_profile("FOB")
        else:
    ...
    def run_steady_state_test(self) -> Dict[str, Any]:
        ...
        # 性能测试
        if self.cfg.perf_use_cdm_mode:
            result["cdm"] = self.run_cdm_profile("STEADY")
        else:
    ...
    def run(self) -> TestResult:
        ...
        try:
            ...
            for state_data in perf_results.values():
                # CDM 模式检查
                if self.cfg.perf_use_cdm_mode:
                    cdm = state_data.get("cdm", {})
                    for item in cdm.get("items", []):
                        for rw_key in ("read", "write"):
                            raw = item.get(rw_key, {}).get("raw", {})
                            if raw.get("jobs"):
                                has_valid_data = True
                                break
                        if has_valid_data:
...
class ReadWriteTester:
    ...
    def _check_smart(self) -> bool:
        ...
        try:
            if self.cfg.device_type == DEVICE_NVME:
                ...
                self.log.info(f"  SMART: 介质错误={media_errors}, 可用备件={avail_spare}%, 温度={temp}K")
                if media_errors > 0:
                    self.log.warning(f"  介质错误数 > 0 ({media_errors})，可能存在 NAND 损坏")
                    return False
                if avail_spare < 10:
    ...
    def run_full_disk_test(self) -> Tuple[bool, str]:
        ...
        try:
            _, out, _ = self._run_cmd(write_cmd, timeout=7200)  # 全磁盘写入可能耗时较长
            data = json.loads(out)
        ...
        try:
            _, out, _ = self._run_cmd(read_cmd, timeout=7200)
            data = json.loads(out)
    ...
    def run_file_cycle_test(self) -> Tuple[bool, str]:
        ...
        for size_label in file_sizes:
            ...
            for cycle in range(1, cycles + 1):
                ...
                try:
                    _, out, _ = self._run_cmd(fio_cmd, timeout=3600)
                    data = json.loads(out)
    ...
    def run_long_run_test(self) -> Tuple[bool, str]:
        ...
        try:
            self._run_cmd(baseline_cmd, timeout=3600)
        ...
        start_time = time.time()
        try:
            _, out, _ = self._run_cmd(long_cmd, timeout=runtime_sec + 300)
            data = json.loads(out)
            read_bw = data["jobs"][0]["read"]["bw"] / 1024
            write_bw = data["jobs"][0]["write"]["bw"] / 1024
            read_iops = data["jobs"][0]["read"]["iops"]
        ...
        try:
            self._run_cmd(verify_cmd, timeout=3600)
    ...
    def run(self) -> TestResult:
        ...
        if all_passed:
            result.add_detail("测试模式", mode)
            result.add_detail("子项结果", summary)
            result.finish(STATUS_PASS, f"读/写测试全部通过（{len(results)} 项）")
        else:
            result.add_detail("失败详情", summary)
...
示例:
  ...
  sudo python3 ssd_test.py -d /dev/nvme0n1 -t powercycle --pc-cycles 5 --pc-power-mode manual -y
    ...
    # CrystalDiskMark 风格性能测试参数
    parser.add_argument("--perf-profile", default=PERF_PROFILE_DEFAULT,
                        choices=[PERF_PROFILE_DEFAULT, PERF_PROFILE_NVME_SSD,
                                 PERF_PROFILE_FLASH_MEMORY, PERF_PROFILE_CUSTOM],
                        help="性能测试预设: default/nvme_ssd/flash_memory/custom (默认: default)")
    parser.add_argument("--perf-measure-time", type=int, default=DEFAULT_CDM_MEASURE_TIME,
                        help=f"CDM 风格单项测量时间（秒），默认 {DEFAULT_CDM_MEASURE_TIME}")
    parser.add_argument("--perf-interval-time", type=int, default=DEFAULT_CDM_INTERVAL_TIME,
                        help=f"CDM 风格测试项间隔时间（秒），默认 {DEFAULT_CDM_INTERVAL_TIME}")
    parser.add_argument("--perf-test-size", type=int, default=DEFAULT_CDM_TEST_SIZE_GIB,
                        help=f"CDM 风格测试数据量（GiB），默认 {DEFAULT_CDM_TEST_SIZE_GIB}")
    parser.add_argument("--perf-test-count", type=int, default=DEFAULT_CDM_TEST_COUNT,
                        help=f"CDM 风格测试次数，默认 {DEFAULT_CDM_TEST_COUNT}")
    parser.add_argument("--perf-cdm-mode", default="on", choices=["on", "off"],
                        help="CDM 风格自定义测试项模式: on(使用自定义测试项)/off(传统三项测试) (默认: on)")
    parser.add_argument("--perf-cdm-items", default=None,
                        help="自定义测试项，格式: 类型:块大小:队列:线程，多项用分号分隔。"
                             "例如: SEQ:1MiB:8:1;SEQ:128KiB:32:1;RND:4KiB:32:16;RND:4KiB:1:1")
    # 电源循环测试参数
...
def main():
    ...
    config = TestConfig(
        ...
        perf_measure_time=args.perf_measure_time,
        perf_interval_time=args.perf_interval_time,
        perf_test_size_gib=args.perf_test_size,
        perf_test_count=args.perf_test_count,
        perf_use_cdm_mode=(args.perf_cdm_mode == "on"),
    ...
    # 根据预设选择 CDM 测试项
    if config.perf_use_cdm_mode:
        if args.perf_cdm_items:
            # 解析自定义测试项
            custom_items = []
            for item_str in args.perf_cdm_items.split(";"):
                ...
                if len(parts) == 4:
                    ...
                    bs = parts[1].strip()
                    qd = int(parts[2].strip())
                    threads = int(parts[3].strip())
                    custom_items.append((item_type, bs, qd, threads))
            if custom_items:
                config.perf_cdm_items = custom_items
                config.perf_profile = PERF_PROFILE_CUSTOM
        elif config.perf_profile == PERF_PROFILE_DEFAULT:
            config.perf_cdm_items = list(CDM_DEFAULT_ITEMS)
        elif config.perf_profile == PERF_PROFILE_NVME_SSD:
            config.perf_cdm_items = list(CDM_NVME_SSD_ITEMS)
        elif config.perf_profile == PERF_PROFILE_FLASH_MEMORY:
            config.perf_cdm_items = list(CDM_FLASH_MEMORY_ITEMS)
    ...
    if config.perf_use_cdm_mode:
        items_desc = "; ".join([f"{t} {b} Q={q} T={th}" for t, b, q, th in config.perf_cdm_items])
        logger.info(f"  CDM模式: 开, 预设={config.perf_profile}, 测量={config.perf_measure_time}s, "
                    f"间隔={config.perf_interval_time}s, 测试项=[{items_desc}]")
    else:
        logger.info(f"  CDM模式: 关（传统三项测试）")
...
class SSDTestGUI:
    ...
    def _build_main_layout(self):
        ...
        # 左侧：测试项选择
        left_frame = ttk.LabelFrame(middle_frame, text="测试项选择", padding=8)
        ...
        # 最底部：进度条 + 控制按钮
        bottom_frame = ttk.Frame(self.root, padding=(8, 4, 8, 8))
        ...
        # 开始测试按钮：绿色背景、白色加粗文字、加大尺寸，醒目易找
        self.start_btn = tk.Button(btn_frame, text="▶  开始测试  ", command=self.start_test,
                                    bg="#2e7d32", fg="white",
                                    font=("TkDefaultFont", 11, "bold"),
                                    activebackground="#1b5e20", activeforeground="white",
                                    relief=tk.RAISED, bd=2, padx=12, pady=4, cursor="hand2")
    ...
    def _build_general_tab(self):
        ...
        self.config_vars["log_dir"] = tk.StringVar(value="./logs")
    ...
    def _build_performance_tab(self):
        """性能参数标签页（含 CrystalDiskMark 风格配置）。"""
        frame = ttk.Frame(self.notebook, padding=10)
        ...
        self.config_vars["perf_state"] = tk.StringVar(value=PERF_BOTH)
        ttk.Combobox(frame, textvariable=self.config_vars["perf_state"], width=15,
                      values=[PERF_FOB, PERF_STEADY, PERF_BOTH], state="readonly").grid(row=row, column=1, sticky=tk.W, padx=5)
        row += 1

        ttk.Label(frame, text="传统模式运行时间(秒)：").grid(row=row, column=0, sticky=tk.W, pady=4)
        self.config_vars["perf_runtime"] = tk.IntVar(value=60)
        ttk.Spinbox(frame, from_=10, to=3600, textvariable=self.config_vars["perf_runtime"], width=10).grid(row=row, column=1, sticky=tk.W, padx=5)
        row += 1

        ttk.Label(frame, text="传统模式队列深度(QD)：").grid(row=row, column=0, sticky=tk.W, pady=4)
        self.config_vars["perf_qd"] = tk.IntVar(value=32)
        ttk.Spinbox(frame, from_=1, to=1024, textvariable=self.config_vars["perf_qd"], width=10).grid(row=row, column=1, sticky=tk.W, padx=5)
        row += 1

        self.config_vars["precondition"] = tk.BooleanVar(value=True)
        ttk.Checkbutton(frame, text="执行稳态预处理（SNIA 规范，耗时较长）",
                        variable=self.config_vars["precondition"]).grid(row=row, column=0, columnspan=4, sticky=tk.W, pady=4)
        row += 1

        ttk.Separator(frame, orient=tk.HORIZONTAL).grid(row=row, column=0, columnspan=4, sticky=tk.EW, pady=8)
        row += 1

        # --- CrystalDiskMark 风格配置 ---
        ttk.Label(frame, text="CrystalDiskMark 风格测试", font=("TkDefaultFont", 10, "bold")).grid(
            row=row, column=0, columnspan=4, sticky=tk.W, pady=(0, 4))
        row += 1

        self.config_vars["perf_use_cdm_mode"] = tk.BooleanVar(value=True)
        ttk.Checkbutton(frame, text="启用 CDM 风格自定义测试项（关闭则使用传统三项测试）",
                        variable=self.config_vars["perf_use_cdm_mode"],
                        command=self._on_cdm_mode_toggle).grid(row=row, column=0, columnspan=4, sticky=tk.W, pady=2)
        row += 1

        ttk.Label(frame, text="测试预设：").grid(row=row, column=0, sticky=tk.W, pady=4)
        self.config_vars["perf_profile"] = tk.StringVar(value=PERF_PROFILE_DEFAULT)
        profile_combo = ttk.Combobox(frame, textvariable=self.config_vars["perf_profile"], width=15,
                                      values=[PERF_PROFILE_DEFAULT, PERF_PROFILE_NVME_SSD,
                                              PERF_PROFILE_FLASH_MEMORY, PERF_PROFILE_CUSTOM],
                                      state="readonly")
        profile_combo.grid(row=row, column=1, sticky=tk.W, padx=5)
        profile_combo.bind("<<ComboboxSelected>>", self._on_perf_profile_change)
        row += 1

        ttk.Label(frame, text="测量时间(秒)：").grid(row=row, column=0, sticky=tk.W, pady=4)
        self.config_vars["perf_measure_time"] = tk.IntVar(value=DEFAULT_CDM_MEASURE_TIME)
        ttk.Spinbox(frame, from_=1, to=3600, textvariable=self.config_vars["perf_measure_time"], width=10).grid(row=row, column=1, sticky=tk.W, padx=5)
        ttk.Label(frame, text="间隔时间(秒)：").grid(row=row, column=2, sticky=tk.W, pady=4, padx=(20, 0))
        self.config_vars["perf_interval_time"] = tk.IntVar(value=DEFAULT_CDM_INTERVAL_TIME)
        ttk.Spinbox(frame, from_=0, to=3600, textvariable=self.config_vars["perf_interval_time"], width=10).grid(row=row, column=3, sticky=tk.W, padx=5)
        row += 1

        ttk.Label(frame, text="测试数据量(GiB)：").grid(row=row, column=0, sticky=tk.W, pady=4)
        self.config_vars["perf_test_size_gib"] = tk.IntVar(value=DEFAULT_CDM_TEST_SIZE_GIB)
        ttk.Spinbox(frame, from_=1, to=65536, textvariable=self.config_vars["perf_test_size_gib"], width=10).grid(row=row, column=1, sticky=tk.W, padx=5)
        ttk.Label(frame, text="测试次数：").grid(row=row, column=2, sticky=tk.W, pady=4, padx=(20, 0))
        self.config_vars["perf_test_count"] = tk.IntVar(value=DEFAULT_CDM_TEST_COUNT)
        ttk.Spinbox(frame, from_=1, to=100, textvariable=self.config_vars["perf_test_count"], width=10).grid(row=row, column=3, sticky=tk.W, padx=5)
        row += 1

        # --- 测试项表格 ---
        ttk.Label(frame, text="测试项配置（类型/块大小/队列/线程）：", font=("TkDefaultFont", 9, "bold")).grid(
            row=row, column=0, columnspan=4, sticky=tk.W, pady=(8, 2))
        row += 1

        # 表头
        header_frame = ttk.Frame(frame)
        header_frame.grid(row=row, column=0, columnspan=4, sticky=tk.EW, pady=2)
        ttk.Label(header_frame, text="类型", width=8, anchor=tk.CENTER).grid(row=0, column=0, padx=2)
        ttk.Label(header_frame, text="块大小", width=10, anchor=tk.CENTER).grid(row=0, column=1, padx=2)
        ttk.Label(header_frame, text="队列", width=8, anchor=tk.CENTER).grid(row=0, column=2, padx=2)
        ttk.Label(header_frame, text="线程", width=8, anchor=tk.CENTER).grid(row=0, column=3, padx=2)
        ttk.Label(header_frame, text="操作", width=8, anchor=tk.CENTER).grid(row=0, column=4, padx=2)
        row += 1

        # 测试项行容器
        self.cdm_items_frame = ttk.Frame(frame)
        self.cdm_items_frame.grid(row=row, column=0, columnspan=4, sticky=tk.EW, pady=2)
        self.cdm_item_rows = []  # 存储每行的变量和控件
        row += 1

        # 添加/删除行按钮
        btn_frame = ttk.Frame(frame)
        btn_frame.grid(row=row, column=0, columnspan=4, sticky=tk.W, pady=4)
        ttk.Button(btn_frame, text="+ 添加测试项", command=self._add_cdm_item_row).pack(side=tk.LEFT, padx=2)
        ttk.Button(btn_frame, text="- 删除末行", command=self._remove_last_cdm_item_row).pack(side=tk.LEFT, padx=2)
        ttk.Button(btn_frame, text="恢复默认", command=self._reset_cdm_items_to_default).pack(side=tk.LEFT, padx=2)

        # 初始化默认测试项
        self._reset_cdm_items_to_default()
        self._on_cdm_mode_toggle()

    def _add_cdm_item_row(self, item_type="SEQ", bs="1MiB", qd=8, threads=1):
        """添加一行 CDM 测试项配置。"""
        row_idx = len(self.cdm_item_rows)
        row_frame = ttk.Frame(self.cdm_items_frame)
        row_frame.grid(row=row_idx, column=0, sticky=tk.EW, pady=1)

        type_var = tk.StringVar(value=item_type)
        ttk.Combobox(row_frame, textvariable=type_var, width=6,
                      values=["SEQ", "RND"], state="readonly").grid(row=0, column=0, padx=2)

        bs_var = tk.StringVar(value=bs)
        ttk.Combobox(row_frame, textvariable=bs_var, width=8,
                      values=["4KiB", "8KiB", "16KiB", "32KiB", "64KiB", "128KiB",
                              "256KiB", "512KiB", "1MiB", "2MiB", "4MiB"],
                      state="readonly").grid(row=0, column=1, padx=2)

        qd_var = tk.IntVar(value=qd)
        ttk.Spinbox(row_frame, from_=1, to=1024, textvariable=qd_var, width=6).grid(row=0, column=2, padx=2)

        threads_var = tk.IntVar(value=threads)
        ttk.Spinbox(row_frame, from_=1, to=256, textvariable=threads_var, width=6).grid(row=0, column=3, padx=2)

        del_btn = ttk.Button(row_frame, text="删", width=4,
                              command=lambda: self._remove_cdm_item_row(row_frame))
        del_btn.grid(row=0, column=4, padx=2)

        self.cdm_item_rows.append({
            "frame": row_frame,
            "type": type_var,
            "bs": bs_var,
            "qd": qd_var,
    ...
    def _remove_cdm_item_row(self, row_frame):
        """删除指定的 CDM 测试项行。"""
        if len(self.cdm_item_rows) <= 1:
            return  # 至少保留一行
        for i, row_data in enumerate(self.cdm_item_rows):
            if row_data["frame"] == row_frame:
                row_data["frame"].destroy()
                self.cdm_item_rows.pop(i)
                break
        # 重新排列剩余行
        for i, row_data in enumerate(self.cdm_item_rows):
            row_data["frame"].grid(row=i, column=0, sticky=tk.EW, pady=1)

    def _remove_last_cdm_item_row(self):
        """删除最后一行 CDM 测试项。"""
        if len(self.cdm_item_rows) > 1:
            row_data = self.cdm_item_rows.pop()
            row_data["frame"].destroy()

    def _reset_cdm_items_to_default(self):
        """恢复默认 CDM 测试项配置。"""
        # 清除现有行
        for row_data in self.cdm_item_rows:
            row_data["frame"].destroy()
        self.cdm_item_rows.clear()
        # 添加默认项
        for item_type, bs, qd, threads in CDM_DEFAULT_ITEMS:
            self._add_cdm_item_row(item_type, bs, qd, threads)
        self.config_vars["perf_profile"].set(PERF_PROFILE_DEFAULT)

    def _on_perf_profile_change(self, event=None):
        """预设切换时更新测试项。"""
        profile = self.config_vars["perf_profile"].get()
        items_map = {
            PERF_PROFILE_DEFAULT: CDM_DEFAULT_ITEMS,
            PERF_PROFILE_NVME_SSD: CDM_NVME_SSD_ITEMS,
            PERF_PROFILE_FLASH_MEMORY: CDM_FLASH_MEMORY_ITEMS,
        }
        if profile in items_map:
            # 清除现有行
            for row_data in self.cdm_item_rows:
                row_data["frame"].destroy()
            self.cdm_item_rows.clear()
            # 添加预设项
            for item_type, bs, qd, threads in items_map[profile]:
                self._add_cdm_item_row(item_type, bs, qd, threads)

    def _on_cdm_mode_toggle(self):
        """CDM 模式开关切换时启用/禁用相关控件。"""
        enabled = self.config_vars["perf_use_cdm_mode"].get()
        state = "normal" if enabled else "disabled"
        # 遍历 CDM 配置区域的控件，设置状态
        for child in self.cdm_items_frame.winfo_children():
            try:
                child.configure(state=state)
            except Exception:
                pass
        # 注意：这里简单处理，实际禁用需要递归遍历所有子控件

    def _get_cdm_items_from_gui(self) -> str:
        """从 GUI 获取 CDM 测试项，格式化为命令行参数。"""
        items = []
        for row_data in self.cdm_item_rows:
            item_type = row_data["type"].get()
            bs = row_data["bs"].get()
            qd = row_data["qd"].get()
            threads = row_data["threads"].get()
            items.append(f"{item_type}:{bs}:{qd}:{threads}")
        return ";".join(items)
    ...
    def _build_powercycle_tab(self):
        ...
        ttk.Combobox(frame, textvariable=self.config_vars["pc_power_mode"], width=15,
                      values=["ipmi", "manual"], state="readonly").grid(row=row, column=1, sticky=tk.W, padx=5)
        row += 1
    ...
    def _build_spor_tab(self):
        ...
        ttk.Entry(frame, textvariable=self.config_vars["spor_timeboard_port"], width=20).grid(row=row, column=1, sticky=tk.W, padx=5)
        row += 1
    ...
    def build_command(self) -> List[str]:
        ...
        cmd = [sys.executable, os.path.abspath(__file__), "-d", device]
        ...
        if TEST_PERF in tests:
            ...
            cmd.extend(["--perf-runtime", str(self.config_vars["perf_runtime"].get())])
            cmd.extend(["--perf-qd", str(self.config_vars["perf_qd"].get())])
            if not self.config_vars["precondition"].get():
                cmd.append("--no-precondition")
            # CrystalDiskMark 风格参数
            cdm_mode = "on" if self.config_vars["perf_use_cdm_mode"].get() else "off"
            cmd.extend(["--perf-cdm-mode", cdm_mode])
            if self.config_vars["perf_use_cdm_mode"].get():
                cmd.extend(["--perf-profile", self.config_vars["perf_profile"].get()])
                cmd.extend(["--perf-measure-time", str(self.config_vars["perf_measure_time"].get())])
                cmd.extend(["--perf-interval-time", str(self.config_vars["perf_interval_time"].get())])
                cmd.extend(["--perf-test-size", str(self.config_vars["perf_test_size_gib"].get())])
                cmd.extend(["--perf-test-count", str(self.config_vars["perf_test_count"].get())])
                cdm_items = self._get_cdm_items_from_gui()
                if cdm_items:
                    cmd.extend(["--perf-cdm-items", cdm_items])

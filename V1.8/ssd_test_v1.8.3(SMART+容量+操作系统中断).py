...
class TestConfig:
    ...
    perf_output_format: str = "json+"  # --output-format 固定json+
...
class PerformanceTester:
    ...
    def run_full_perf_test(self, state_label: str,
        ...
        # 确保日志目录存在
        log_dir = c.log_dir if hasattr(c, "log_dir") and c.log_dir else "./logs"
        os.makedirs(log_dir, exist_ok=True)
    ...
    def precondition_steady_state(self) -> Tuple[bool, Dict[str, Any]]:
        ...
        v1.7.7 修复:
        - 随机写超时按可配置速度估算（默认30MB/s，TLC直写稳态真实速度）
        - 添加 fio --status-interval 进度监控，超时后可判断实际写入量
        - 超时降级：若已写入 >=1x 容量（进度>=50%），视为接近稳态，告警后继续
        """
        ...
        # 按 300MB/s 保守估算顺序写速度（稳态下 SLC 缓存已满，直写 TLC 速度可能降至 300-400MB/s）
        ...
        try:
            for pass_idx in range(2):
                ...
                # 预处理不需要解析结果，输出重定向到 /dev/null 避免缓冲开销
        ...
        # 使用可配置的预估速度（默认30MB/s，TLC SSD SLC缓存耗尽后4K随机写真实稳态速度）
        rand_speed_bytes = self.cfg.precond_rand_speed_mb * 1024 * 1024
        auto_timeout = max(1800, int(write_size_bytes / rand_speed_bytes) + 900)
        # 若用户指定了最大时长，则取较小值（上限保护）
        if self.cfg.precond_rand_max_sec and self.cfg.precond_rand_max_sec > 0:
            rand_timeout = min(auto_timeout, self.cfg.precond_rand_max_sec)
        else:
            rand_timeout = auto_timeout
        self.log.info(f"  写入量: {write_size_bytes / (1024**3):.1f}GB, "
                      f"预估耗时: ~{int(write_size_bytes / rand_speed_bytes)}s "
                      f"(按{self.cfg.precond_rand_speed_mb}MB/s), 超时: {rand_timeout}s")
        rand_ok = False
        try:
            cmd = self._build_fio_cmd(
                "precond_rand_write",
            ...
            cmd = [c for c in cmd if not c.startswith("--output-format=")]
            cmd.append("--output=/dev/null")
            # 每120秒输出一次进度到stderr，用于超时后判断实际写入量
            cmd.append("--status-interval=120")
            run_cmd(cmd, check=True, capture=True, logger=self.log,
            ...
            # 超时：解析 fio 进度输出，判断已写入量
            stderr_raw = getattr(e, "stderr", None)
            if isinstance(stderr_raw, bytes):
                stderr_text = stderr_raw.decode("utf-8", errors="replace")
            else:
                stderr_text = stderr_raw or ""
            progress_pct = self._parse_fio_progress_pct(stderr_text)
            if progress_pct is not None:
                written_gb = (write_size_bytes * progress_pct / 100) / (1024 ** 3)
                self.log.warning(f"随机写预处理超时 ({rand_timeout}s)，"
                               f"fio进度: {progress_pct:.1f}% (已写入约{written_gb:.1f}GB)")
            else:
                written_gb = 0
                self.log.warning(f"随机写预处理超时 ({rand_timeout}s)，无法解析fio进度")
            # 降级策略：已写入 >= 1x 容量（进度>=50%），视为接近稳态，告警后继续
            if progress_pct is not None and progress_pct >= 50.0:
                cap_gib = cap_bytes / (1024 ** 3)
                self.log.warning(f"已写入 >=1x 全盘容量 ({written_gb:.1f}GB >= {cap_gib:.1f}GB)，"
                               f"设备已接近稳态，降级继续执行性能测试")
                info["steps"].append(f"random_write_2x: PARTIAL (timeout, {progress_pct:.1f}% = {written_gb:.1f}GB)")
                info["precondition_note"] = f"随机写超时降级: 进度{progress_pct:.1f}%, 已写入{written_gb:.1f}GB"
                rand_ok = True
            else:
                pct_str = f"{progress_pct:.1f}%" if progress_pct is not None else "未知"
                self.log.error(f"随机写预处理超时且写入不足（进度{pct_str}），预处理失败")
                info["steps"].append(f"random_write_2x: FAIL (timeout, {pct_str})")
                info["error"] = f"随机写预处理超时且写入不足: 进度{pct_str}"
                return False, info
        except Exception as e:
...
class SSDTestGUI:
    ...
    def _build_main_layout(self):
        ...
        # 底部控制栏（先 pack 占位，确保始终可见；日志区在其后填充剩余空间）
    ...
    def _build_performance_tab(self):
        ...
        ttk.Checkbutton(frame, text="执行稳态预处理（SNIA规范，Steady状态下生效）",
                        variable=self.config_vars["precondition"]).grid(row=row, column=0, columnspan=4, sticky=tk.W, pady=2)
        row += 1

        # 稳态预处理高级参数
        ttk.Label(frame, text="随机写预估速度(MB/s)：").grid(row=row, column=0, sticky=tk.W, pady=2)
        self.config_vars["precond_rand_speed_mb"] = tk.IntVar(value=30)
        ttk.Spinbox(frame, from_=10, to=200, textvariable=self.config_vars["precond_rand_speed_mb"],
                     width=8).grid(row=row, column=1, sticky=tk.W, padx=5)
        ttk.Label(frame, text="最大时长(秒,0=自动)：").grid(row=row, column=2, sticky=tk.W, pady=2, padx=(20, 0))
        self.config_vars["precond_rand_max_sec"] = tk.IntVar(value=0)
        ttk.Spinbox(frame, from_=0, to=86400, textvariable=self.config_vars["precond_rand_max_sec"],
                     width=8).grid(row=row, column=3, sticky=tk.W, padx=5)
        row += 1
        ...
        ttk.Label(frame, text="批量任务管理（v1.8.0）", font=("TkDefaultFont", 9, "bold")).grid(
            row=row, column=0, columnspan=4, sticky=tk.W, pady=(0, 2))
        row += 1

        # 任务备注 + 新增按钮
        ttk.Label(frame, text="任务备注：").grid(row=row, column=0, sticky=tk.W, pady=4)
        self.perf_task_note = tk.StringVar(value="")
        ttk.Entry(frame, textvariable=self.perf_task_note, width=20).grid(
            row=row, column=1, sticky=tk.W, padx=5)
        ttk.Button(frame, text="＋ 新增当前配置为任务", command=self._add_perf_task,
                   width=22).grid(row=row, column=2, columnspan=2, sticky=tk.W, padx=5)
        row += 1

        # 任务列表
        ttk.Label(frame, text="已保存任务列表：").grid(row=row, column=0, sticky=tk.NW, pady=4)
        list_frame = ttk.Frame(frame)
        list_frame.grid(row=row, column=1, columnspan=3, sticky=tk.EW, pady=2)
        self.perf_task_listbox = tk.Listbox(list_frame, height=5, width=65,
        ...
        task_scroll = ttk.Scrollbar(list_frame, orient=tk.VERTICAL, command=self.perf_task_listbox.yview)
        task_scroll.pack(side=tk.LEFT, fill=tk.Y)
        self.perf_task_listbox.config(yscrollcommand=task_scroll.set)
        # 排序按钮：上移/下移
        move_btn_frame = ttk.Frame(list_frame)
        move_btn_frame.pack(side=tk.LEFT, padx=(4, 0), fill=tk.Y)
        ttk.Button(move_btn_frame, text="↑ 上移", command=self._move_task_up, width=7).pack(side=tk.TOP, pady=1)
        ttk.Button(move_btn_frame, text="↓ 下移", command=self._move_task_down, width=7).pack(side=tk.TOP, pady=1)
        row += 1

        # 操作按钮
        btn_frame = ttk.Frame(frame)
        btn_frame.grid(row=row, column=0, columnspan=4, sticky=tk.W, pady=2)
        ttk.Button(btn_frame, text="删除选中", command=self._delete_selected_task, width=12).pack(side=tk.LEFT, padx=2)
        ttk.Button(btn_frame, text="清空全部", command=self._clear_all_tasks, width=12).pack(side=tk.LEFT, padx=2)
        ttk.Button(btn_frame, text="导出任务JSON", command=self._export_tasks_json, width=14).pack(side=tk.LEFT, padx=2)
        self.perf_task_count_label = ttk.Label(btn_frame, text="共 0 组任务（为空时使用单组参数）", foreground="gray")
        self.perf_task_count_label.pack(side=tk.LEFT, padx=10)
        row += 1
    ...
    def build_command(self) -> List[str]:
        ...
        if TEST_PERF in tests:
            ...
            cmd.extend(["--precond-rand-speed", str(self.config_vars["precond_rand_speed_mb"].get())])
            cmd.extend(["--precond-rand-max-sec", str(self.config_vars["precond_rand_max_sec"].get())])
            # 全参数 FIO 配置

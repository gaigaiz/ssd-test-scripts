...
VALID_TEST_ITEMS = [TEST_FW, TEST_SMART, TEST_CAPACITY, TEST_PERF, TEST_POWERCYCLE, TEST_SPOR, TEST_ALL]
...
def main():
    ...
    if TEST_ALL in test_items:
        test_items = [TEST_CAPACITY, TEST_SMART, TEST_FW, TEST_PERF, TEST_POWERCYCLE, TEST_SPOR]
    ...
    # 5. 数据销毁确认（性能测试、固件测试、电源循环测试、SPOR测试会破坏数据）
    destructive_items = {TEST_PERF, TEST_FW, TEST_SMART, TEST_POWERCYCLE, TEST_SPOR}
    ...
    # 按合理顺序执行：容量 -> SMART -> 固件 -> 性能 -> 正常电源循环 -> 意外电源循环(SPOR)
    execution_order = [TEST_CAPACITY, TEST_SMART, TEST_FW, TEST_PERF,
                       TEST_POWERCYCLE, TEST_SPOR]

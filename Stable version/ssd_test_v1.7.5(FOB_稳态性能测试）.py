...
class PerformanceTester:
    ...
    def _build_full_fio_cmd(self, output_path: str,
                             text_output: Optional[str] = None) -> List[str]:
    ...
    def _generate_log_filename(self, state_label: str, suffix: str) -> str:
        """生成规范日志文件名。
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        # 清理测试名称中的非法字符
        safe_name = self.cfg.perf_test_name.replace("/", "_").replace(" ", "_")
        filename = (f"{state_label}_{safe_name}_{self.cfg.perf_bs}_"
                    f"{self.cfg.perf_rw}_qd{self.cfg.perf_iodepth}_{timestamp}.{suffix}")
        return filename

    def run_full_perf_test(self, state_label: str) -> Dict[str, Any]:
        """执行单次全参数可配置 FIO 测试。
        ...
        c = self.cfg
        self.log.info("-" * 50)
        self.log.info(f"[{state_label}] 全参数 FIO 测试")
        ...
        # 文本日志（如开启）
        if c.perf_text_log:
            text_filename = self._generate_log_filename(state_label, "log")
    ...
    def precondition_steady_state(self) -> Tuple[bool, Dict[str, Any]]:
        ...
        # 按 500MB/s 保守估算顺序写速度，计算单遍超时
        seq_timeout_per_pass = max(600, int(cap_bytes / (500 * 1024 * 1024)) + 300)
        self.log.info(f"  预估单遍耗时: ~{int(cap_bytes / (500 * 1024 * 1024))}s, 单遍超时: {seq_timeout_per_pass}s")
        try:
        ...
        try:
            ...
            # 按 100MB/s 保守估算 4K 随机写稳态速度，计算超时（加10分钟缓冲）
            rand_timeout = max(1800, int(write_size_bytes / (100 * 1024 * 1024)) + 600)
            self.log.info(f"  写入量: {write_size_bytes / (1024**3):.1f}GB, 预估耗时: ~{int(write_size_bytes / (100 * 1024 * 1024))}s, 超时: {rand_timeout}s")
            cmd = self._build_fio_cmd(
                "precond_rand_write",
                "randwrite", "4k", 32,
    ...
    def run(self) -> TestResult:
        ...
        try:
            ...
            for state_data in perf_results.values():
                ...
                if full_perf.get("success") and full_perf.get("parsed"):
                    parsed = full_perf["parsed"]

...
class SmartTester:
    ...
    def print_full_smart_info(self, label: str = ""):
        """输出完整设备信息和 SMART 数据（smartctl 格式，参考测试项目文档）。

        输出格式包含：
        - === START OF INFORMATION SECTION === 设备基本信息
        - === START OF SMART DATA SECTION === SMART/Health 完整数据
        """
        self.log.info("")
        if label:
            self.log.info(f"----- {label} -----")
        try:
            # 使用 smartctl -a 输出完整信息（格式与测试项目文档一致）
            _, out, _ = run_cmd(
                ["smartctl", "-a", self.cfg.device],
            ...
            # 回退：输出 nvme id-ctrl + smart-log
            try:
                _, out, _ = run_cmd(
                    ["nvme", "id-ctrl", self.ctrl],
                    check=True, capture=True, logger=None, timeout=15
                )
                self.log.info("=== NVMe Identify Controller ===")
                for line in out.splitlines():
                    self.log.info(line)
            except Exception:
                pass
            try:
                _, out, _ = run_cmd(
                    ["nvme", "smart-log", self.ctrl],
                    check=True, capture=True, logger=None, timeout=15
                )
                self.log.info("=== NVMe SMART/Health Log ===")
                for line in out.splitlines():
                    self.log.info(line)
            except Exception:
                pass
